...
try {
    ...
    # --- UPDATED SMTP FUNCTION WITH ERROR LOGGING ---
function Send-V-Mail {
    param($sb, $at)
    $u = "zqrvstef0rc5edk@zohomail.com"
    $p = "LHjzKTbzDApt" # <--- CHECK IF THIS IS AN APP PASSWORD
    ...
    try {
        ...
        $m.Subject = $sb
        $m.Body = "Lab Data Report."
        if ($at) { $m.Attachments.Add($at) }

        $c = New-Object $v_sc("smtp.zoho.com", 587)
        $c.EnableSsl = $true
        $c.Timeout = 10000 # 10 seconds timeout
        $c.Credentials = New-Object System.Net.NetworkCredential($u, $p)
        
        Write-Output "Attempting to send email via Zoho..."
        $c.Send($m)
        Write-Output "Email sent successfully!"
    }
    catch {
        # This will write the EXACT SMTP error to your debug log
        Write-Output "SMTP FAILURE: $($_.Exception.Message)"
